...
public class Picture extends SimplePicture 
{
  ...
  public void fixUnderwater() {
	  ...
	  // take a sample from a fish and find average color value and range of colors
	  for (int row = 26; row < 36; row++) {
		  for (int col = 178; col < 198; col++) {
			  totalPix++;
			  Pixel myPixel = pixels[row][col];
			  rAvg += myPixel.getRed();
			  gAvg += myPixel.getGreen();
			  bAvg += myPixel.getBlue();
			  
			  if (myPixel.getRed() > maxR) {
				  maxR = myPixel.getRed();
			  }
			  if (myPixel.getRed() < minR) {
				  minR = myPixel.getRed();
			  }
			  if (myPixel.getGreen() > maxG) {
				  maxG = myPixel.getGreen();
			  }
			  if (myPixel.getGreen() < minG) {
				  minG = myPixel.getGreen();
			  }
			  if (myPixel.getBlue() > maxB) {
				  maxB = myPixel.getBlue();
			  }
			  if (myPixel.getBlue() < minB) {
				  minB = myPixel.getBlue();
			  }
		  }
	  }
	  rAvg = rAvg / totalPix;
	  ...
	  double maxDistance = Math.sqrt(redDistance * redDistance + greenDistance * greenDistance + blueDistance * blueDistance);
  ...
  public void mirrorDiagonalOpp() { // mirrors from top left to bottom right
      Pixel[][] pixels = this.getPixels2D();
      Pixel topRightPixel = null;
      Pixel bottomLeftPixel = null;
      int maxLength;
